...
import 'package:streamhub/Secciones/Info_Peliculas.dart';

class PeliculasView extends StatefulWidget {
  const PeliculasView({super.key});

  @override
  _PeliculasViewState createState() => _PeliculasViewState();
}
...
class _PeliculasViewState extends State<PeliculasView> {
  ...
  Future<void> _loadMoreMovies() async {
    ...
    try {
      _currentPage++;
      final moreMovies = await TMDBService.getPopularContent(isMovie: true, page: _currentPage);
      if (mounted) {
        setState(() {
      ...
      print("Error cargando más películas: $e");
    }
  }
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        title: Text('Películas', style: TextStyle(color: isDark ? Colors.white : Colors.black, fontSize: 24)),
  ...
  Widget _buildMovieCard(MediaItem movie) {
